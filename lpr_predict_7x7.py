...
boxes4 = Conv2D(n_boxes * 4, feature_map_shape, strides=(1, 1), padding="same", kernel_initializer='he_normal', name='boxes4')(x)
anchors4 = AnchorBoxes(input_shape[0], input_shape[1], this_scale=0.1428, next_scale=0.1428, aspect_ratios=[1.0],
...
batch_size=1
encoder = SSDInputEncoder(input_shape[0], input_shape[1], class_count, clip_boxes=clip_boxes, two_boxes_for_ar1=False, aspect_ratios_global=[1.0], predictor_sizes=[feature_map_shape], min_scale=0.1428, max_scale=0.1428)

data_augmentation_chain = DataAugmentationConstantInputSize(random_flip=0, random_scale=(0.8, 1.01,0.5))
...
ext_model.load_weights('lpr_train_7x7.h5', by_name=True)
ext_model.summary()
img, label=next(generator)

img=utils.load_img('try.png', (224,224)) #load image for prediction
img =np.expand_dims(img, axis=0)

print(label.shape)
y_pred=ext_model.predict(img)
print(y_pred.shape)
print(np.reshape(np.argmax(y_pred[0,:,0:4], axis=1), feature_map_shape))
